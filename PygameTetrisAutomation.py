...
class Tetromino(pygame.sprite.Sprite):
        ...
        for brick in self.bricks: #This moves the origin block for some specific scenarios of J,L and I
            if brick.origin == True:
                if self.type ==1 and (self.rotationAngle == 90 or self.rotationAngle == 270):
                	x = brick.rect.x + int(screen_width/number_of_columns)
                	y = brick.rect.y
    ...
    def update(self, pressed_keys, Wall, score, level, number_Of_Rows_With_No_Downkey):
        ...
        if pressed_keys[K_LEFT]:
            for brick in self.bricks:
                ...
                for wallBrick in Wall.bricks:
                    if brick.rect.x - int(screen_width/number_of_columns) == wallBrick.rect.x:
                        if brick.rect.y == wallBrick.rect.y:
                            self.leftEdge = True
            if self.leftEdge == False and self.wallOverlap == False:
                for brick in self.bricks:
                    brick.rect.x = brick.rect.x - int(screen_width/number_of_columns)
        ...
        if pressed_keys[K_DOWN]:
            for brick in self.bricks:
                if brick.rect.y >  number_Of_Rows_With_No_Downkey*int(screen_height/number_of_rows):
    ...
    def drop_Tetromino(self, Wall):
        ...
        if self.wallOverlap == True:
            for brick in self.bricks:
                New_Brick = generic_Brick(brick.rect.x,brick.rect.y, brick.red, brick.green, brick.blue, False)
                Wall.bricks.append(New_Brick)
        elif self.wallOverlap == False:
            for brick in self.bricks:
                brick.rect.y = brick.rect.y + int(screen_height/number_of_rows)
...
class wallClass(pygame.sprite.Sprite):
    ...
    def clearLines(self, level, score):
    	...
    	for i in range(0,number_of_rows):
    	    for brick in self.bricks:
    	        if brick.rect.y == screen_height - (i+1)*int(screen_width/number_of_columns):
    	...
    	for i in range(0,number_of_rows):
    	    if self.bricksperLine[i] >= number_of_columns:
    	        self.bricks = [brick for brick in self.bricks if brick.rect.y !=(screen_height - (i+1)*int(screen_width/number_of_columns))]
    	...
    	while len(self.completeRows) > 0:
    	    for i in range(self.completeRows[-1]+1,number_of_rows):
    	        for brick in self.bricks:
    	            if brick.rect.y < screen_height - (i)*int(screen_width/number_of_columns):
    	        	    brick.rect.y = brick.rect.y + int(screen_height/number_of_rows)
